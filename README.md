...
Processing 12M rows natively in Python/Pandas or SQL engines often causes Out-Of-Memory (OOM) errors on low-spec hardware. The following engineering patterns were enforced:
    ...
    Turbocharger Standardization & R&D Allocation:
                   ...
                   Over 48% of total global revenue is generated by Black and White vehicles alone. Plant logistics should pre-allocate baseline paint supplies for monochrome finishes while implementing made-to-order (MTO) agile workflows for low-volume colors (Orange, Brown, Yellow) to eliminate carrying costs.  
                   
                   Post-2023 Market Correction Strategy:
                   
                   With volumes normalizing from the 2023 peak (~2.3M) to ~1.9M units, marketing incentives should pivot from broad-market push strategies to targeted retention and upselling toward higher-tier trims (S-Class, GLS, and AMG variants) to protect gross margins. 
                   
                   Data Infrastructure Scalability:
                   
                   Maintain the established low-footprint data governance model. Leveraging chunked ingestion and indexed SQL views ensures that expanding historical datasets past 50M+ rows will not degrade cloud compute budgets or breach RAM boundaries.
